...
- **Card state management.** Approving, denying, rejecting individual cards happens through inline callout buttons (see [`obsidian-ui/callouts.md`](callouts.md)) or through the Kanban directly. There's no `Memoria: approve this card` command because card state changes need card context the palette doesn't provide. (Card buttons live in the [`obsidian-ui/callouts.md`](callouts.md) callouts.)
