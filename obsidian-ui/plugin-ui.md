...
The named components — [dashboards](dashboards.md), [callouts](callouts.md), [command palette](command-palette.md), [status line](status-line.md), [Home](home.md), the [Agent Client pane](../obsidian-plugins/required/agent-client.md) — are each *powered by* a specific plugin. This page is the rest: the **additional UI affordances** the required and recommended plugins add to the editor, search, reading, and visual layers that aren't one of those components.

Scope: **required + recommended only.** [`reference/`](../obsidian-plugins/README.md) plugins (obsidian-kanban, ZotLit, zotero-integration, obsidian-linter) are documented but **not installed**, so they contribute no UI here. One required plugin — `obsidian-local-rest-api` — is **headless** (the HTTP bridge to Hermes) and renders nothing.
...
| --- | --- | --- |
| [Dashboards](dashboards.md), [Status line](status-line.md), [Home](home.md) queries | `dataview` | required |
| [Callouts](callouts.md) (`[!brief]` / `[!suggestions]` / `[!verification]`) | `callout-manager` | required |
| [Command palette](command-palette.md) (`Memoria:` actions) | `quickadd` | required |
| [Agent Client pane](../obsidian-plugins/required/agent-client.md) | `agent-client` | required |
| [Home](home.md) auto-open | `obsidian-homepage` | recommended |
...
| Plugin | Tier | What it adds |
| --- | --- | --- |
| `obsidian-style-settings` | rec | A settings panel exposing CSS-snippet / theme variables as toggles and sliders — tune the look (accent color, spacing) without editing CSS. The GUI behind the [design system](design-system.md). |
...
| Plugin | Tier | What it adds |
| --- | --- | --- |
| `obsidian-git` | req | A **Source Control** sidebar (stage / commit / diff) and a **status-bar** sync indicator — the visible surface of the git layer the [Verify](../workflows/downstream/verify.md) / [Revise](../workflows/downstream/revise.md) hooks ride on. |

## Design notes

- **Affordances, not chrome.** Every entry adds a *capability* (a search, a popover, a colored link), not decoration. That restraint is the [ui-discipline](ui-discipline.md) rule applied to plugins: one accent color, hidden chrome, no visual noise.
- **Headless ≠ UI.** `obsidian-local-rest-api` is required but renders nothing — it's the control-plane bridge, not a surface. Don't look for it in the UI.
- **Reference plugins are absent by design.** If you're looking for Kanban-board rendering or ZotLit's import UI, they're [`reference/`](../obsidian-plugins/README.md) — evaluated, not installed (the Hermes board renders via Dataview, not obsidian-kanban).
- **The catalog of *which plugins exist and how to configure them* lives in [obsidian-plugins/](../obsidian-plugins/README.md).** This page is the *UI-facing* view of that set; that folder is the *install-and-config* view.
