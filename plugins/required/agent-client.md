...
| --- | --- | --- | --- |
| **Socratic** — Think a source through in conversation before you distill it. | `memoria-socratic` | **Persistent pane (default)** | Long conversations during processing. Architecturally write-denied (`policy.allow.write: []`); safe on any device. The standard ACP use case — open the pane in the Reading & Processing workspace, talk through a paper note via questioning. |
...
- **Librarian** — network-active; every `find`-style chat costs external API calls. Better dispatched via cards (queued, audited, retryable). Add only if you specifically want interactive discovery and accept the cost characteristics.
