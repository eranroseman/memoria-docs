...
- **Board memory is per-card, not per-profile.** When a card moves from the Librarian lane to the Writer lane, the handoff payload travels with it (see [kanban-board/card-schema.md](../kanban-board/card-schema.md#structured-payload)). The Writer does not inherit the Librarian's working context — only the payload.
