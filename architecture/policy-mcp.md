...
Profile permissions and lane scopes are not just documentation — they are enforced at the tool layer by a **policy MCP** that intercepts every vault write. This document is the detailed contract; the summary lives in [architecture/README.md](README.md).
...
The policy MCP's contract — action vocabulary, four-decision protocol, SHA-256 audit hashes, skill-conditional composition — is what the rest of Memoria depends on: the [M-series drift detectors](../profiles/linter.md), the [audit-log dashboard](../dashboards/audit-log.md), the inline-callout writes, the reversibility guarantee. Those aren't features Memoria could drop to use Tirith alone; they're how Memoria's design holds together.
...
- [architecture/README.md](README.md) — three-layer model the MCP sits inside
