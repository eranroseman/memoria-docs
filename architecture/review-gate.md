...
- **At the MCP level** — every write to a review-gated zone returns `dry_run` regardless of the requesting profile's lane policy. The write is logged and reported but not performed; the worker must surface the proposed change as a board comment for human approval. See [architecture/policy-mcp.md](policy-mcp.md#the-decision-protocol).
...
Every write to any of these paths degrades to `dry_run` at the policy MCP, for every profile. The policy MCP's `review_gated.dry_run` rule sits above lane-override configuration — no lane can grant itself write access to a review-gated zone. See [profiles/README.md](../profiles/README.md#lane-permissions-matrix) for the per-profile view of this constraint.
...
- [architecture/policy-mcp.md](policy-mcp.md) — the full decision protocol and `dry_run` response contract
