...
| **Writer** | Draft and synthesize | `draft`, `query`, `lint`, `promote` (handoff) | `llm-wiki draft`, `note-refactor`, `scientific-writing`, `counter-outline` | `search_web`, `fetch_url`, vault read/write | `rest-passthrough`, external-API skills, publish-review-gated | `10-inbox/02-answers/`, `40-workbench/*/04-drafts/`, `40-workbench/*/02-framing/` (via `counter-outline`) |
...
| **Coder** | Code artifacts | `code`, `commit`, `revert`, `workspace`, `scaffold` | `scaffold-code-note`, `workspace-coordinate`, `commit-and-document` (thin Hermes-side wrappers; substantive coding work lives in the external coding agent — see [Coder ↔ external coding agent](#coder--external-coding-agent)) | Git, filesystem, repo APIs | review-gated-zone edits, prose ownership | `40-workbench/*/06-code/` |
