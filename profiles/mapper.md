...
- **Not Librarian.** Librarian fetches new sources from the outside world; Mapper maps what's already there. They share retrieval tooling but Mapper cannot reach external APIs at all (`external_api_policy: blocked`).
...
Folder permission matrix lives in [profiles/README.md](../profiles/README.md#folder-permission-matrix); the runtime contract (full command list, deterministic/hybrid annotations per command, exit conditions) lives in the SOUL.md.

## Related

- Workflows: [assess](../workflows/downstream/assess.md), [refactor](../workflows/maintenance/refactor.md), [ingest §`comparative-brief`](../workflows/upstream/ingest.md)
- Pilot: [E1 Open Notebook](../roadmap/pilots/01-open-notebook.md) — active experiment using Open Notebook as the LLM back-end for narrative composition in `comparative-brief`.
...
- Reference: [architecture/computational-toolbox.md](../architecture/computational-toolbox.md) — the embedding, clustering, and search primitives Mapper relies on.

<!-- memoria-nav -->

---

[← Previous: Librarian — design summary](librarian.md)

[Next: Socratic — design summary →](socratic.md)
