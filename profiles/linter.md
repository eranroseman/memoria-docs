...
Folder permission matrix lives in [profiles/README.md](README.md#folder-permission-matrix); the runtime contract (lint check table, full M-detector specs, threshold values) lives in the SOUL.md and M-detectors.md alongside it. The command catalog is in [profiles/profile-commands.md](profile-commands.md).
...
- ADRs: [9 typed relations](../decisions/09-typed-relations-frontmatter.md) — the Linter validates `relations:` keys against the controlled vocabulary (a `schema-check` concern). [16 contradictions dashboard](../decisions/16-contradictions-dashboard.md) — adjacent concept; the Linter is structural, the contradictions surface is content.
