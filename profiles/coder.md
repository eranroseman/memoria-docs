...
- **Confined to `40-workbench/01-projects/*/code/` and project pages.** Coder's writes never touch `20-sources/`, `30-synthesis/01-claims/`, or any canonical zone — even as a side effect. This is enforced at the policy MCP layer, not just by convention. Code changes that need to update synthesis or sources go through Writer + human review, not through Coder.
...
- **Repo-vs-vault routing rule.** Small scripts that belong to the vault live in `40-workbench/01-projects/*/code/`; larger projects earn their own repo and live outside the vault, with a `code-note` in the vault as the index. The threshold and the `repo:` frontmatter convention live in [profiles/why-coder-external-agent.md](../profiles/why-coder-external-agent.md).

## Permissions and commands

Folder permission matrix lives in [profiles/README.md](../profiles/README.md#folder-permission-matrix); the runtime contract lives in the SOUL.md. Notably narrow: five commands (`code`, `commit`, `revert`, `workspace`, `scaffold`), mostly thin wrappers around git and the code-note template.

## Related

- Workflows: [code](../workflows/downstream/code.md)
- ADRs: [06 code-agent attachment](../decisions/06-code-agent-attachment.md), [10 code-artifact autopilot](../decisions/10-code-artifact-autopilot.md)
- Rationale: [why-coder-external-agent.md](why-coder-external-agent.md) — the authoritative doc on the two-agent boundary, the `files.readonlyInclude` convention, and where projects live (vault vs own repo).
...
[← Previous: Verifier — design summary](verifier.md)

[Next: Linter — design summary →](linter.md)
