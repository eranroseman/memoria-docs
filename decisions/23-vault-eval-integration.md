...
# ADR-23: vault-eval as a maintenance capability
...
- **Surfacing (observability).** Results append to `00-meta/08-metrics/eval/` and trend on a dashboard. The verdict is **diagnostic, not gating** — unlike `drift-watch`'s structural FAIL, an eval dip informs the human; it does not pause scheduled work.
- **Gold set (vault).** Gold tasks live in `00-meta/05-eval/` as YAML; they become a dedicated note type only if the [expansion-threshold](../roadmap/README.md#expansion-threshold-discipline) is tripped.
...
- Diagnostic-only means a capability regression won't auto-halt work; the human must notice it on the dashboard. This is intentional, per [success-metrics.md](../roadmap/success-metrics.md) ("diagnostic, not contract").
