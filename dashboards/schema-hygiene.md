...
Catch leftover junk that accumulates between rituals: filenames containing `TODO`, `tmp`, or `untitled` that the agent or human forgot to finish or rename. Run after ingest batches or whenever something feels off. The dashboard surfaces; the human chooses the action per file (rename, finish, archive, delete).
...
[← Previous: reading-pipeline — design summary](reading-pipeline.md)

[Next: skill-lifecycle — design summary →](skill-lifecycle.md)
