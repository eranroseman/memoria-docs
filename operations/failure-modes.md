...
Operational reference for failures that block core workflows. Each entry follows a **Detect / Fix / Verify** triplet: the symptom an operator notices, the commands to recover, and the check that confirms the recovery actually worked. This is the canonical companion to `00-meta/safe-mode.md` (the vault-resident operator note); use safe-mode for the *first response* and this doc for the *full recovery procedure*.
...
**Detect.** `hermes -p memoria-researcher run llm-wiki ingest --source {citekey}` returns "citekey not found" or "not in library.bib".

```bash
grep {citekey} 00-meta/03-config/library.bib   # should return one entry
```
...
```bash
grep {citekey} 00-meta/03-config/library.bib            # entry present
git log --oneline 00-meta/03-config/library.bib | head -3   # recent commit
hermes -p memoria-researcher run llm-wiki ingest --source {citekey} --dry-run   # no "not found" error
```

### 2. Missing `_draft_classification` — triage can't proceed

**Detect.** Note exists in `20-sources/01-literature/` with `triage_status: partial` but no `_draft_classification` comment block in the file body. The triage query in [weekly-dashboard](../dashboards/weekly-dashboard.md) surfaces the note but it has no classification proposal to review.

**Fix.**

```bash
hermes -p memoria-researcher run triage --source {citekey}
# Re-runs triage on a single note; re-proposes classification from the abstract
```

**Verify.** Open the note — `_draft_classification` comment block now present with `study_design`, `methods`, `topic`, and `projects` fields populated. `triage_status` remains `partial` (correct — it only becomes `full` after the human promotes).
...
In Obsidian: Properties panel shows no error; note appears in a Dataview query (`FROM "20-sources/01-literature" WHERE file.name = "{citekey}"`).
...
**Detect.** Linter M1 (profile install drift) reports a SHA-256 mismatch between `.memoria/profiles/memoria-<name>/<file>` (vault source) and `~/.hermes/profiles/memoria-<name>/<file>` (deployed copy).
...
```bash
hermes -p memoria-linter run health-report --detectors M1
# No drift reported
```
...
| --- | --- | --- | --- |
| **Obsidian Linter corrupts frontmatter** | CRITICAL | Linter running on agent-maintained folders | Exclude `10-inbox/`, `20-sources/`, `30-synthesis/02-wiki/` in linter `foldersToIgnore` — see [obsidian-plugins/README.md](obsidian-plugins/README.md). |
| **`_draft_classification` or `_enrichment` deleted** | CRITICAL | A linter rule or plugin upgrade introduced an HTML-comment stripper that ran on save | **Never enable any rule that strips HTML comments.** Currently no such rule exists in obsidian-linter v1.31.2, but the discipline is forward-looking — diff the rule registry before accepting any plugin upgrade. See [obsidian-plugins/README.md](obsidian-plugins/README.md). |
...
| Dataview queries returning nothing | HIGH | `study_design` or `topic` vocabulary inconsistency — query returns empty table that looks like "nothing to do" | Check values in notes match the schema vocabulary exactly (see [schema.md](../reference/schema.md)). |
...
| `_draft_classification` not appearing | MEDIUM | `triage` skill not installed or not in lane's allow list | `hermes skills install triage`; check `.memoria/lane-overrides/research.yaml` |
| Syncthing + `.bib` race condition | MEDIUM | VPS reads `.bib` while Syncthing is mid-transfer | Use Git pull for `.bib` distribution on Option C, not Syncthing — see [07-roadmap.md](../07-roadmap/sync-and-coordination.md#bib-watcher-option-c-only). |
...
| Cron job didn't fire overnight | MEDIUM | Sleep-prone host or stale `.env` | Deployment Option C only (VPS); check `journalctl --user -u hermes-overnight` and the [overnight loop section](../07-roadmap/future-directions.md#the-overnight-loop-proactive-discovery-pattern). |
| Retry count climbing on same card | MEDIUM | Brittle prompt or broken tool | After `retry_count > 3` the card auto-escalates to `blocked-on-human` — see [03-board.md retry pattern](../03-board.md#retry-pattern). The operator decides whether to revise the packet or close as infeasible. |
...
| Bitwarden bootstrap token rejected | LOW | Wrong region or revoked token | Re-run `hermes secrets bitwarden setup` and pick the correct region (US Cloud / EU Cloud / self-hosted) — see [07-roadmap.md secret management](../07-roadmap/secret-management.md). |

## When this doc is wrong

If a failure recurs and the Fix here doesn't work, treat that as a design issue. Either the symptom mapping is incomplete (the doc points at the wrong cause) or the fix is brittle (works sometimes but not always). In both cases the right response is to update this doc, not to memorize a workaround — the next operator who hits it should find a recipe that actually works.
