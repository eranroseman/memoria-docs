...
3. **Type distinction** — Luhmann distinguished *fleeting notes* (raw capture), *literature notes* (what a source says), and *permanent notes* (your own durable claim). Each serves a different function and has a different lifespan. Memoria preserves this three-way distinction but renames the corresponding types (`source-note`, `claim-note`) for clarity in a software context — see [05-notes-folders.md](05-notes-folders.md).
...
A survey of recent agent-driven research systems — LitSearch, ResearchArena, SciLitLLM, LitLLM, LatteReview, ResearchAgent, Idea2Story, AI-Scientist (v1 and v2), AgentLaboratory, Sibyl, AutoResearchClaw, AI-Researcher, Auto-Research, OmegaWiki, CORAL, AIDE ML — provided the concrete patterns that turn the Karpathy / Zettelkasten / Memex stack into a runnable system. From these:

- **Stage-gated pipelines** (LitSearch, ResearchArena, MLR-Copilot) — distinct stages with distinct outputs and validation points.
- **Explicit agent roles** (AI-Scientist v2, LatteReview, Sibyl) — separate planners, executors, reviewers, writers.
- **Persistent knowledge graphs** (OmegaWiki, Idea2Story) — typed relationships as primary memory, not RAG-only.
- **Reviewable organization artifacts** (LitLLM, LatteReview) — synthesis as inspectable output, not hidden in prompts.

What Memoria *rejects* from these systems is just as defining as what it borrows — see [rationale/pattern-provenance.md](rationale/pattern-provenance.md) for the borrow / adapt / ignore table and [01-architecture/why-no-autonomous-synthesis.md](01-architecture/why-no-autonomous-synthesis.md) for the three boundaries that block adoption of autonomous-scientist patterns.
...
- The associative graph (wikilinks, MOCs, entity links) preserves trails (Memex). wikilink
...
- A system where **the human review state is structurally blocking, not advisory.** A card cannot promote until the operator changes `review_state` to `approved`. This is deliberate positioning: the surveyed contemporary autonomous-research systems (ResearchAgent, AI co-scientist, MOOSE, Agent Laboratory) use LLM-based reviewers that advise; few make a human gate structurally required. Memoria's blocking gate is the structural form of the human-judgment commitment.

## Position on the autonomy spectrum

Chen 2026 (*From Copilots to Colleagues: A Survey of Autonomous Research Agents*) proposes a five-level taxonomy: **L1** code autocomplete, **L2** multi-step assistant with operator approval per step, **L3** multi-step autonomous execution under operator-set strategy with per-batch review, **L4** self-directed within a bounded domain (the current frontier — AI Scientist, AiScientist, AI co-scientist), and **L5** fully self-directed research agendas (aspirational).

**Memoria targets L3 with a structurally enforced ceiling.** Profiles execute multi-step unattended within a card (Researcher discovers, ingests, enriches, classifies; the overnight loop runs nightly; the Coder lane runs experiment loops against scalar criteria), but the operator sets the strategy (`research-directions.md`, `screening-protocol.md`) and the review state gates promotion. Within L3, Memoria pursues the within-boundary autonomy progression documented in [07-roadmap/autonomy-progression.md](07-roadmap/autonomy-progression.md).

Memoria deliberately does not target L4 or L5. The boundary is structural: [01-architecture/why-no-autonomous-synthesis.md](01-architecture/why-no-autonomous-synthesis.md) refuses the scalar-metric optimization and autonomous keep/revert patterns that L4 systems rely on, because synthesis correctness is not scalar and synthesis changes are not reversible. The single exception is the Coder lane (where Chen 2026's three preconditions actually hold) — see [§"Scope: these boundaries apply to synthesis"](01-architecture/why-no-autonomous-synthesis.md#scope-these-boundaries-apply-to-synthesis).
...
- **Not a Deep Research agent.** "Deep Research" agents (OpenAI DR, Gemini DR, Perplexity DR, Grok DeepSearch) are query-driven and ephemeral — produce a comprehensive report per query, then end. Memoria is corpus-curating and durable — the operator builds a vault over months, and each session compounds with prior sessions. The two categories serve different operator needs; Memoria is the wrong tool for one-shot reports and the right tool for long-horizon research substrates. See [Huang et al. 2025](../papers/) and [Xu & Peng 2025](../papers/) for the DR-category taxonomy.

## Patterns rejected

Memoria's adopted patterns are listed under [Intellectual foundations](#contemporary-ai-research-systems) above; the full borrow / adapt / ignore breakdown lives in [rationale/pattern-provenance.md](rationale/pattern-provenance.md). What Memoria *refuses* to adopt is defining in its own right:

- **Full open-ended autonomous scientist mode** — too broad, misaligned with the human-review philosophy. Memoria's [autonomy boundary](01-architecture/why-no-autonomous-synthesis.md) is structural, not configurable.
...
- **Folders encode lifecycle stage, not subject area.** A source note about HCI lives in `20-sources/01-literature/`, not in `HCI/`. Topics belong in frontmatter and links.
- **Canonical synthesis is human-owned.** `30-synthesis/01-permanent/` is not auto-written. Agents can suggest links; humans write claims.
- **The review state must change before promotion.** No worker says "I'm finished" and that promotes the card. The operator changes `review_state` to `approved`.
- **Source notes are never overwritten by structure or code agents.** Provenance is preserved.
...
If any of those is unfamiliar, [05-notes-folders.md](05-notes-folders.md) and [06-surfaces.md](06-surfaces.md) will give the most concrete grounding; [01-architecture.md](01-architecture.md) and [02-profiles.md](02-profiles.md) require more background.

## Next

- For the architecture: [01-architecture.md](01-architecture.md).
- For day-one operational workflows: [04-workflows.md](04-workflows.md).
