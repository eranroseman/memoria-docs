...
3. [kanban-board/](kanban-board/) — The control plane: states, review gate, handoff schema.
...
7. [obsidian-ui/](obsidian-ui/) — How the human interacts: dashboards, workspaces, callouts, status line, command palette, Agent Client.
...
- **Need a recipe to do something?** → [workflows/](workflows/), [operations/](operations/), [obsidian-plugins/](obsidian-plugins/).
- **Looking something up?** → [glossary.md](glossary.md), and the precise reference files inside each topic folder ([kanban-board/states.md](kanban-board/states.md), [kanban-board/card-schema.md](kanban-board/card-schema.md), [vault/frontmatter-schema.md](vault/frontmatter-schema.md), [vault/templates.md](vault/templates.md), [vault/linking-patterns.md](vault/linking-patterns.md), [profiles/profile-commands.md](profiles/profile-commands.md), [obsidian-ui/command-palette.md](obsidian-ui/command-palette.md), [architecture/policy-mcp.md](architecture/policy-mcp.md), [architecture/computational-toolbox.md](architecture/computational-toolbox.md), `obsidian-ui/*.md`, `dashboards/*.md`).
...
| [obsidian-ui/](obsidian-ui/) | `README.md` (the Obsidian UI components) + per-component detail (`persistent.md` dashboards, `modal.md` workspaces, `inline.md` callouts, `ambient.md` status line) + `design-system.md` (visual-style template) + `command-palette.md`. |
| [dashboards/](dashboards/) | `README.md` (folder index) + 11 per-dashboard design summaries (`daily-health.md` is the entry point). Runtime Dataview queries live at `00-meta/01-dashboards/` in the [starter vault](https://github.com/eranroseman/memoria-vault). |
| [operations/](operations/) | `README.md` + [failure-modes.md](operations/failure-modes.md) (Detect / Fix / Verify recipes). |
| [obsidian-plugins/](obsidian-plugins/) | `README.md` (priority-ordered overview) + per-plugin configuration split by lifecycle into `required/` (8, system breaks without them), `recommended/` (10, quality-of-life installs), `reference/` (1, held-knowledge / not in the install set), plus top-level `plugin-configs-lifecycle.md` and `ui-discipline.md` (Obsidian UI conventions). |
| [decisions/](decisions/) | 23 architecture decision records (ADRs) — 20 with their own file, 3 (ADR-1, 8, 11) index-only — + `_template.md` + `by-topic.md` (secondary index grouping ADRs by topic folder) + `adopt-on-demand-for-reviews.md` (shared rationale for the four deferred systematic-review ADRs 12 / 18 / 19 / 20). Cross-cuts all topics, so it sits at the top level. |
