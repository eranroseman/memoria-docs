...
Profile permissions and lane scopes are not just documentation — they are enforced at the tool layer by a **policy MCP** that intercepts every vault write. This document is the detailed contract; the summary lives in [01-architecture.md](../01-architecture.md). For the relationship to Hermes's Tirith policy primitive and framework portability, see [§ Relationship to Hermes Tirith](#relationship-to-hermes-tirith) at the bottom.

## Why a runtime gate

Prompts are advisory. A profile that *should* only write to `10-inbox/` can be coaxed by a malformed task, a hallucinated path, or an overreaching delegation into writing somewhere it shouldn't. The review gate (see [03-board.md](../03-board.md)) catches this at the human-decision level, but the policy MCP catches it at the file-system level — before the write happens, not after.

This is what makes the lane-override files (see [02-profiles.md](../02-profiles.md#lane-override-files)) executable rather than aspirational.
...
| `dry_run` | Action is logged and reported but not performed; the worker should escalate to a board comment for human approval. Always logged. | Path is in a canonical zone (`30-synthesis/01-permanent/`, `30-synthesis/03-moc/`, `50-deliverables/`) even if the profile is otherwise allowed. |
...
The MCP guards eight distinct actions. The distinction matters: a researcher with `write` access to `10-inbox/` should not be able to `delete` from it, and a linter with `auto_fix` should not be able to `write` arbitrary content.
...
| `auto_fix` | Linter-only repair action. | Class-gated by `flags.class` (see [linter.md](../profiles/linter.md#auto-fix-policy)). |
...
Lane-overrides are the baseline policy a profile carries through a session. A small set of skills need to *tighten* that baseline. The currently-shipped example: `counter-outline` (loaded by Writer during the Frame stage) must narrow Writer's write scope from `40-workbench/02-drafts/**` to `40-workbench/01-projects/*/framing/**` only — so the operator can't accidentally use Writer-with-counter-outline to write actual drafts. Skill-conditional policy is the mechanism.

(For why `socratic-processing` and `lens-reading` are now native to the Socratic profile rather than restrictive skills — and the one-way promotion rule that motivates the choice — see the [restrictive-skills note in 02-profiles.md](../02-profiles.md#skills-with-restrictive-policy).)
...
One skill currently ships with a skill-conditional `policy.deny` block: `counter-outline` (Writer-loaded; scratch-only writes to `40-workbench/01-projects/*/framing/`). The catalog and rationale live in [02-profiles.md](../02-profiles.md#skills-with-restrictive-policy). New restrictive skills go through the same onboarding checklist as any other skill (see [07-roadmap.md](../07-roadmap/skill-governance.md#skill-onboarding-checklist)); the additional review is that the deny rules don't contradict the host lane's allow rules into uselessness.
...
```json
{
  ...
  "action": "write",
  "path": "40-workbench/03-code/project-x/main.py",
...
- `flags.draft_only` permits a soft write to a canonical zone that lives outside the canonical file (e.g., a board comment proposing a change).
...
{
  "decision": "deny",
  "policy_rule": "writer.deny.30-synthesis-permanent",
  "message": "memoria-writer cannot write to canonical permanent notes"
}
```
...
Every decision — `allow`, `allow_with_log`, `deny`, or `dry_run` — is appended to `00-meta/04-logs/audit.jsonl` as one JSON object per line:
...
```json
{
  ...
  "action": "write",
  "path": "10-inbox/02-synthesis/2026-05-25-effect-of-X.md",
  "task_id": "TASK-2026-05-25-014",
  "decision": "allow",
...
- **Tamper detection.** If a file's current hash doesn't match the last `after_hash` recorded for its path, the file was modified outside the audit trail — either by a non-MCP write or by direct hand-editing. The linter surfaces these mismatches (see [linter.md](../profiles/linter.md)).
...
The append-only JSONL format means the trail survives crashes, is grep-friendly, and feeds the [audit-log dashboard](../dashboards/audit-log.md) directly. The linter is responsible for weekly rotation (see [linter.md](../profiles/linter.md)).
...
The policy MCP's contract — action vocabulary, four-decision protocol, SHA-256 audit hashes, skill-conditional composition — is what the rest of Memoria depends on: the [M-series drift detectors](../profiles/linter.md#structural-detectors-and-verdicts), the [audit-log dashboard](../dashboards/audit-log.md), the inline-callout writes, the reversibility guarantee. Those aren't features Memoria could drop to use Tirith alone; they're how Memoria's design holds together.

**Framework portability.** Because the enforcement contract is documented and self-contained, Memoria's design works against any MCP-capable agent — Hermes, Claude Code, Aider, custom clients. Operators using Hermes can leave Tirith enabled; operators running Memoria against a different agent framework only need the policy MCP.
...
- [01-architecture.md](../01-architecture.md) — three-layer model the MCP sits inside
- [02-profiles.md](../02-profiles.md#lane-override-files) — the lane-override YAML files the MCP reads
- [linter.md](../profiles/linter.md) — the linter's auto-fix policy classes the MCP enforces; vault hash drift detection
- [audit-log dashboard](../dashboards/audit-log.md) — operator view of decisions
