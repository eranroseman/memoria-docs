...
> **Status: deferred — not the current architecture.** Memoria's current distribution model is [direct profile management](../01-architecture/on-disk-layout.md): the seven profile directories under `.memoria/profiles/memoria-<name>/` are hand-authored and copied verbatim to `~/.hermes/profiles/` by the install script. There is no compiler today.
>
...
This reference is the canonical mechanism. The architecture overview in [01-architecture.md](../01-architecture.md) summarizes; this document specifies.
...
│   └── profiles/
│       ├── researcher.yaml       # per-profile override: model, commands, soul sections, tool filters
│       ├── cartographer.yaml
│       ├── socratic.yaml
│       ├── writer.yaml
│       ├── verifier.yaml
...
8. Write compiled outputs into `~/.hermes/profiles/memoria-<name>/`:
   ...
   - `mcp.json` — Hermes-canonical MCP server connections (mirrors `mcp_servers:` in `config.yaml`).
   ...
   - `.env.EXAMPLE` — required and optional env vars, commented out, with descriptions. The operator copies this to `.env` and fills in real values on first install.
   - `policy.manifest.yaml` — Memoria-specific extension: the effective policy manifest the policy MCP reads at startup. Not part of Hermes's canonical profile shape; Hermes ignores it.
...
| Unknown tool | A profile references a tool not present in `00-meta/03-config/tool-registry.yaml`. |
| Missing secret | A profile's `.env` declares a key that isn't supplied by the secret manager. |
| Unknown command | A profile defines a command outside the canonical command set in [02-profiles.md](../02-profiles.md). |
...
Each profile's `build-info.json` records the SHA-256 hashes of (a) every base file and (b) the override file that produced it. The linter's `health-report` check (see [linter.md](../profiles/linter.md#log-rotation)) recomputes these hashes and flags profiles whose compiled outputs no longer match their declared sources. This catches accidental hand-edits to compiled files.
...
5. Optionally commit the regenerated outputs to a `memoria-profiles/` sibling repo or `dist/` subtree if the operator wants the audit trail of effective changes alongside the source diff.
