...
- **`status` is card-only; notes use `lifecycle`.** The board card's `status` is the Hermes Kanban enum (`triage` / `todo` / `ready` / `running` / `blocked` / `done` / `archived` — see [kanban-board/states.md](../kanban-board/states.md)). Notes never carry `status`; their durability phase is `lifecycle` (`proposed` / `current` / `dormant` / `archived`). A note never carries `status` and a card never carries `lifecycle`, so although both value sets include `archived`, the field name alone disambiguates the two. (Memoria's earlier `status` values like `queued`/`delivered`/`accepted` map onto the Hermes enum plus the `metadata.review_status` overlay; see the [state crosswalk](../kanban-board/states.md#crosswalk-old-memoria-states).)
