...
Both blocks are agent-owned (see the namespace table above): the agent writes them, the human promotes from them, and the agent never writes the main YAML directly. **No HTML-comment-stripping Obsidian Linter plugin rule may run** — it would silently delete these blocks (see [obsidian-plugins/reference/obsidian-linter.md](../obsidian-plugins/reference/obsidian-linter.md)).
