...
**Why it's safe.** It issues an open-view action, not a write. It cannot mutate canonical notes, so it raises none of the audit/authority concerns that put [obsidian-linter](../reference/obsidian-linter.md) in `reference/` ([ADR-24](../../decisions/24-obsidian-linter-reference-only.md)). Same test, opposite verdict.
