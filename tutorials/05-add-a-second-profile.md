...
[roadmap/README.md (graduated start)](../roadmap/README.md#implementation-paths-graduated-start) sequences them by felt need. **Mapper** is the common second lane — once the corpus is large enough that "what do I already have on X?" is a real question, the Mapper's `scope-project` and `[!brief]` earn their place.
...
- The Mapper writes only its declared scope (`40-workbench/*/01-map/`); any write outside it is denied by the policy MCP and surfaces on [audit-log](../dashboards/audit-log.md).

## Next

- Add lanes as the need appears (Writer for drafting, Verifier for citation tracing). The [profiles/README.md](../profiles/README.md) is the per-lane reference.
