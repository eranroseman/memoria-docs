...
2. **Ingest each source.** For every citekey:
   ...
   Each run creates a `paper-note` at `lifecycle: proposed` with `_proposed_classification`, enriches via OpenAlex/Crossref, and (for OA PDFs) extracts text to `90-assets/extracts/`. See [workflows/upstream/ingest.md](../workflows/upstream/ingest.md).
3. **Classify each note.** Open each paper-note, review `_proposed_classification`, promote the fields you agree with into the main YAML, delete the proposed block, and set `lifecycle: current` + `triage_completed`. See [workflows/upstream/classify.md](../workflows/upstream/classify.md).
...
- [`reading-pipeline`](../dashboards/reading-pipeline.md) shows the batch moving from `proposed` to `current`.
