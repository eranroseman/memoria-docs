...
> **Status.** Tutorials 02, 03, and 05 exercise the agent pipeline, which depends on the **v0.2 profile wiring** (`config.yaml`, `mcp.json`, `policy_mcp.py`, lane-overrides) not yet in the starter vault — see [implementation-status.md](../implementation-status.md). They are written as the intended flow; tutorial 04 (claim authoring + promotion) is mostly human-driven and works today.
