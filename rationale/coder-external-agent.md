...
The Memoria coder profile and external coding agents (Kilocode, Aider, Claude Code, Codex) operate as **parallel agents with a shared filesystem**. Hermes owns research and wiki; the coding agent owns code. The `code-note` is the human-readable handoff between them.

This reference is the canonical how-to. The summary in [02-profiles.md](../02-profiles.md#coder--external-coding-agent) is the architectural framing; this document specifies the workspace patterns.
...
- The external coding agent reads the vault as a second workspace folder (read-only via `external_directory`); it edits code in `40-workbench/03-code/`.
...
Open the vault root in VS Code. The coding agent gets direct access to the literature notes and wiki pages that motivated the code. Put the agent's instruction file (`CLAUDE.md`, `.kilocode/rules/`, `.cursorrules`, depending on tool) in `40-workbench/03-code/<project>/` so the agent's context starts there rather than at the vault root.

### Real projects with their own lifecycle

Give the project its own Git repo outside the vault, then open the project repo as the primary workspace folder and add the vault as a *read-only* second folder. The canonical `.code-workspace` pattern:
...
The first folder is the project repo (where the agent reads and writes); the second is the vault (which the agent reads for context but should never modify). Replace `<your-vault>` with the actual folder name the operator cloned the starter vault into. The `files.readonlyInclude` rule enforces the boundary at the editor level — even if the agent has write tools enabled, VS Code refuses the write.

### Rule of thumb

If the project has `requirements.txt`, `pyproject.toml`, `package.json`, or another dependency manifest, it gets its own repo. Single scripts or notebooks live inside the vault at `40-workbench/03-code/<project>/` and open with the vault root.
...
- **Memoria owns content.** Drafts, claim notes, source notes, verification reports, design-system.md.
- **Open-design owns rendering.** Reads Memoria's content + design-system.md as input, produces the polished artifact.
- **The human owns the gate.** Reviews the rendered artifact; accepts to `50-deliverables/` or rejects (with the standard [post-rejection paths](../03-board.md#post-rejection-paths) — spawn a revision render with a different design system, or discard).
...
Replace `<your-vault>` with the actual folder name the operator cloned the starter vault into (per [on-disk-layout.md](../01-architecture/on-disk-layout.md#starter-vault-versioned-distributable), the vault root folder name is operator-defined).
...
- `design_system:` — which design system was used (e.g., `lab-jitai-2026`), referencing the design-system file at `00-meta/06-schema/`
- `render_command:` — the exact open-design command that produced the artifact (for reproducibility)

This keeps the deliverable traceable back into the vault even after it's been exported — a paper PDF in `50-deliverables/` can be traced to the claim notes whose content it presents, the source notes those claims cite, and the design-system that styled it.
...
Memoria's existing **Pandoc export pipeline** ([04-workflows.md](../04-workflows.md)) handles body-text exports (Word, PDF, HTML) for journal submissions, manuscripts, and other text-first deliverables. The boundary is:
...
Open-design's image generation capabilities (gpt-image-2, Seedance, HyperFrames) are powerful but raise reproducibility concerns for research outputs. **Default to disabled in Memoria contexts.** A research deliverable should use the design system's deterministic side (typography, layout, color, spacing) but render real figures, real diagrams, real photos. AI-generated illustrations belong only in contexts where the operator has explicitly opted in (e.g., a popular-science blog post, a teaching slide). The design-system template's anti-patterns section names this explicitly — see [reference/design-system.md](../reference/design-system.md).
