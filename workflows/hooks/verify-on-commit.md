...
When a Writer commits a draft to `40-workbench/<project>/04-drafts/`, a git post-commit hook fires automatically. The hook creates a `verify` card in the Kanban board, placing it in the Verifier's lane queue in `ready` state. This is the entry point to the [Verify](../downstream/verify.md) workflow.
...
5. The human reviews the verdict and advances the card to [Revise](../downstream/revise.md) or export.

The full card lifecycle is documented in [Verify](../downstream/verify.md#card-lifecycle).

## Related

- [Verify workflow](../downstream/verify.md) — what Verifier does once it claims the card
