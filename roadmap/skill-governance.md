...
One markdown note per active skill, kept in `00-meta/07-skills/`. The note carries the authoritative metadata that the [`skill-lifecycle` dashboard](../dashboards/skill-lifecycle.md) queries.
...
`skill-note` is not in the 15 note types listed in [vault/README.md](../vault/README.md) — it's a meta-note in `00-meta/`, the same family as registry and config notes. Don't link skill notes from the synthesis graph.
...
5. **Register permissions** (`testing → needs-review`). Add to the appropriate lane-override file's `policy.allow.skills` list. Add to the lane permissions matrix in [profiles/README.md](../profiles/README.md#lane-permissions-matrix).
