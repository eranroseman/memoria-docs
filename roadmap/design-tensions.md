...
- **Specialization vs. simplicity.** Seven profiles is more setup than one agent. The payoff is reliability, but the cost is configuration overhead. If you can't keep seven profiles' permissions straight, fall back to the four-profile minimal configuration (see [profiles/README.md](../profiles/README.md)) and accept the safety loss.
- **Vault stability vs. graph richness.** A rich knowledge graph is the goal, but every link is also a maintenance burden. The linking patterns in [vault/README.md](../vault/README.md) bias toward fewer, stronger links.
...
[← Previous: Standard cron tasks](standard-cron-tasks.md)
