...
4. **Local full install** only for a *developer* role (see [Per-device install sets in Phase 3](timeline.md#per-device-install-sets)), never for the principal investigator's laptop in human-as-reader mode. The dev install must use `HERMES_HOME` isolation and point at a *test vault*, never the production vault.
